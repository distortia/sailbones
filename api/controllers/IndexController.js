...
module.exports = {
	...
	reset_post: function(req, res){
      ...
      if (found) {
        ...
        req.session.flash = {errorMessage: 'No user found with the email of ' + req.body.email};
		...
		User.findOne({email: req.params.email}, function(err, user){
			...
			if(user.uniqueId === req.body.uniqueId && user.uniqueId != 'undefined'){
				if(req.body.password === req.body.password2){
					...
					req.session.flash = {userMessage: 'Passwords do not match'};
					res.redirect('/reset/' + req.params.email);
				}
			}
			else{
				req.session.flash = {userMessage: 'Invalid unique ID'};
				res.redirect('/reset/' + req.params.email);
			}
		})
	},
